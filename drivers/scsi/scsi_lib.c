...
//Indicate the status of HD (Link up, down or accessing), in drivers/ata/libata-core.c
extern atomic_t sata_hd_islinked;
extern atomic_t esata_hd_islinked;
extern atomic_t sata_hd_accessing;
extern atomic_t esata_hd_accessing;

// Indicate the status of LED, in drivers/ata/libata-core.c
extern atomic_t sata_led_lighting;
extern atomic_t esata_led_lighting;
...
void scsi_io_completion(struct scsi_cmnd *cmd, unsigned int good_bytes)
{
	...
	//Turn on LED
	if(cmd->device->host->unique_id == 1) {
		//SATA
...
int scsi_init_io(struct scsi_cmnd *cmd, gfp_t gfp_mask)
{
	...
	if(cmd->device->host->unique_id == 1) {
		...
		if(atomic_read(&sata_badblock_idf) == 0 && atomic_read(&sata_hd_islinked) == 1) {
#ifdef CONFIG_ZyXEL_STG212
			atomic_set(&sata_hd_accessing, 1);	// set it as accessing
#endif
		}
	}
	else {
