...
#define OXSEMI_FAN_SPEED_RATIO_MAX 	179	/* 70% */
...
static int cold_limit 		= 59;
static int min_fan_speed_ratio  = 0;
...
/* fan speed ratio is controlled by user; default is no */
static atomic_t flag_user_fanspeed = ATOMIC_INIT(0);
...
static int monitor_task(void *arg)
{
	...
	while(!kthread_should_stop()) {
		...
		if (atomic_read(&flag_user_fanspeed) == 0)
			update_fan_speed(th);
...
/* only allow to write fan-speed */
static int
oxsemi_therm_write(struct file *file, const char __user *buffer, unsigned long count, void *data)
{
	char my_buf[5];
	int user_fanspeed;

	if (count > sizeof(my_buf)) {
		printk(KERN_ERR "Fail to write data. Input data length must be smaller than %d.\n", sizeof(my_buf));
		return -EFAULT;
	}

	copy_from_user(my_buf, buffer, count);

	user_fanspeed = simple_strtol(my_buf, NULL, 10);

	if (user_fanspeed > 255)
		user_fanspeed = 255;
	else if (user_fanspeed < 0) {
		atomic_set(&flag_user_fanspeed, 0);
		return count;
	}

	atomic_set(&flag_user_fanspeed, 1);

	write_reg( FAN_SPEED_RATIO_SET, user_fanspeed );

	if (thermostat)
		thermostat->last_speed = user_fanspeed;
